...
//                    }
    ...
    jQuery.fn.extend({
        ...
        addClass:function(str){
            ...
            return this.each(function(){
                var dom=this;
                $.each(classNames,function(i,singleName){
        ...
        removeClass:function(str){//删除一个或多个类名 没有参数就是删除全部类名
            ...
            return  this.each(function(){
                var dom=this;
                var classNameChange=" "+dom.className+" ";

                jQuery.each(classNames,function(i,singleName){
                    var singleChange=" "+singleName+" ";

                    classNameChange= classNameChange.replace(singleChange," ");


                })
                dom.className=classNameChange;
    ...
    jQuery.fn.extend({
        attr:function(){
            //通过设置属性可以映射到页面上
            //当只有一个字符串的参数的时候是获取第一个dom的制定属性
            //有一个对象的实参 是设置每一个dom元素的
            ...
            if(length==0) return this;//没有实参的时候
            if(length==1){
                if(jQuery.isString(arg0)){
                    return this.get(0).getAttribute(arg0)
                }else{
                    return  this.each(function(){
                        var dom=this;
                        jQuery.each(arg0,function(attrName,attrValue){
                            dom.setAtrribute(attrName,attrValue)
                        })
    ...
    jQuery.fn.extend({
        on:function(evenType,callback){
            return  this.each(function(){
                var dom=this;
                dom.addEventListener(evenType,callback)
            })
        },
        off:function(evenType,callback){
            return  this.each(function(){
                var dom=this;
                dom.removeEventListener(evenType,callback)
            })
        },
    })
    //绑定事件
    var evenTypes="click dblclick mouseenter mouseleave mouseover mouseout mousemove keydown keyup keypress load".split(" ");
    jQuery.each(evenTypes,function(i,evenType){

        jQuery.fn[evenType]=function(callback){
            return this.on(evenType,callback)
        }
    })
